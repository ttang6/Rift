...
ZClient::ZClient() : m_zhandle(nullptr) {}
...
void ZClient::start() {
    ...
    std::string host = Application::getConfig().load("zookeeperip");
    std::string port = Application::getConfig().load("zookeeperport");
...
// 创建ZooKeeper节点
void ZClient::create(const char *path, const char *data, int datalen, int state) {
...
// 获取ZooKeeper节点的数据
std::string ZClient::getData(const char *path) {
