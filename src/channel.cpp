                             ...
                             ::google::protobuf::Closure *done)
{
    if (-1 == m_clientfd) {  // 如果客户端socket未初始化
        ...
        ZClient z_client;
        z_client.start();  // 连接ZooKeeper服务器
        std::string host_data = QueryServiceHost(&z_client, service_name, method_name, m_idx);  // 查询服务地址
    ...
    if (rpcheader.SerializeToString(&rpc_header_str)) {  // 序列化头部信息
...
// 创建新的socket连接
bool Channel::newConnect(const char *ip, uint16_t port) {
    // 创建socket
...
// 从ZooKeeper查询服务地址
std::string Channel::QueryServiceHost(ZClient *z_clientent, std::string service_name, std::string method_name, int &idx) {
    ...
    std::string host_data_1 = z_clientent->getData(method_path.c_str());  // 从ZooKeeper获取数据
    lock.unlock();  // 解锁
...
// 构造函数，支持延迟连接
Channel::Channel(bool connectNow) : m_clientfd(-1), m_idx(0) {
